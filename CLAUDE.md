...
- **Blog is a unified "river"**: native posts (in `src/content/writing/`) plus link-posts pulled at build time by `src/loaders/external.ts` and merged in `src/lib/river.ts`. Sources: Defenses in Depth (author-scoped RSS) and Random Lives (HTML scrape) — see "External facts". Podcasts are native link-post `.md` files. The internal content-collection name is `writing` (not user-facing).
...
- **Defenses in Depth** — Damon's blog at defensesindepth.bio, multi-author, source at `niharikasingh/defensesindepth` (Jekyll, Damon has push). The loader reads one author-scoped feed, `/authors/damon-binder/feed.xml`: every post of his, uncapped, in a format a redesign of that site cannot disturb. It skips posts titled `Linkpost: …` — the blog runs its own link-posts, and Damon carries the same links here as native link-posts pointing at the article rather than at the DiD wrapper page.
  - **That feed exists because of this site.** The blog was on Ghost; the August 2026 move to Jekyll dropped `/author/damon/rss/`, the loader fetched a 404, and the river lost every DiD post without a single build failing. The stopgap scraped `/authors/damon-binder/` HTML, which worked but would break the next time the card markup moved. So the feed was added upstream instead ([PR #1](https://github.com/niharikasingh/defensesindepth/pull/1)), and all the scraping came back out. **If more feeds are ever needed there, add them upstream rather than scraping** — `_includes/feed.xml` takes `author` and `limit` parameters, and one five-line page per author is the whole pattern.
  - **A broken source fails the build on purpose.** `externalLoader` throws if any source returns nothing, so the Actions run goes red and sends mail instead of shipping a river with holes in it. Pages keeps serving the last successful deploy, so a red build leaves the live site up and correct. Fetches retry three times first, so a blip does not trigger it. **A failed deploy here usually means an upstream site changed, not that this repo is broken** — read the `[external-posts]` lines in the run.
