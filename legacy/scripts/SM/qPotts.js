(function() {

	//var width = 600, height = 600; // Width and height of simulation in pixels.
	var width = 501, height = 501; // Width and height of simulation in pixels.
	var cellSize = 3; // Size of a cell in pixels.
	var yCellCount = Math.floor(height/cellSize); // Number of cells in the up-down direction.
	var xCellCount = Math.floor(width/cellSize); // Number of cells in the left-right direction.

	function randcell() {
		return Math.floor(Math.random()*xCellCount);
	}


	var context = $("#canvas").get(0).getContext("2d");

	context.canvas.width = width;
	context.canvas.height = height;

	function makeBlankGrid(q) {
		// First, we make a new array of rows. Then we set those rows.
		var grid = new Array(yCellCount);
		for (var i = 0; i < yCellCount; i++) {
			grid[i] = new Array(xCellCount);
			for (var j = 0; j < grid[i].length; j++) {
				grid[i][j] = Math.floor(Math.random()*q);
			}
		}
		return grid;
	};

	var grid = makeBlankGrid(1);
	var temperature = 4;
	var magnetism = 0;
	var q = 1;

	var paused = false

	function drawGrid(grid) {
		var val_to_deg
		for (var y = 0; y < yCellCount; y++) {
			for (var x = 0; x < xCellCount; x++) {
				val_to_deg = Math.round(grid[x][y]/q*360);
				context.fillStyle = "hsl("+val_to_deg+", 80%, 50%)";
				context.fillRect(x*cellSize, y*cellSize, cellSize, cellSize);
			}
		}
	};

	function boltz(x) {
		if (x>0){
			return true
		} else {
			return (Math.exp(x)>Math.random())
		}
	}

	function delta(a,b,c){
		if (a==b) {
			return 1
		} else if (a==c) {
			return -1
		} else {
			return 0
		}
	}

	function neighbours(x,y,b,c) {
		var n = delta(grid[x%xCellCount][(xCellCount+y-1)%yCellCount],b,c);
		n    += delta(grid[x%xCellCount][(y+1)%yCellCount],b,c);
		n    += delta(grid[(x+1)%xCellCount][y%yCellCount],b,c);
		n    += delta(grid[(xCellCount+x-1)%xCellCount][y%yCellCount],b,c);
		if (b==0) {
			n+=magnetism;
		} else if (c==0){
			n-=magnetism;
		}
		return n;
	}

	function mainLoop() {
		context.fillStyle = "rgb(0, 0, 0)";
		context.fillRect(0, 0, width, height);

		// Update slider-based variables.

		temperature = Math.pow(6, parseFloat($("#temperature").val()))-1;
		$("#tempdisplay").html(Math.round(1000*temperature)/1000+"");

		magnetism = parseFloat($("#magnetism").val())
		$("#magdisplay").html(magnetism+"");


		new_q = 1
		for (i = 2; i<11; i++){
			value = "#choose" + i;
			if ($(value).is(':checked')) {
				new_q = i
			}
		}

		if (new_q != q) {
			grid = makeBlankGrid(new_q)
		}
		q = new_q

		// Run simulation step.

		if (!paused) {
			var x0, y0, state, newstate;
			for (var y = 0; y < 10*yCellCount; y++) {
				for (var x = 0; x < xCellCount; x++) {
					x0 = randcell();
					y0 = randcell();
					state = grid[x0][y0]
					newstate = (state + 1 + Math.floor(Math.random()*(q-1)))%q
					if (boltz(neighbours(x0,y0,newstate,state)/temperature)) {
						grid[x0][y0] = newstate
					} 
				}
			}
		}

		drawGrid(grid);

		setTimeout(mainLoop, 0); // Run, run, as fast as you can!
	};
			

	mainLoop();
}).call();